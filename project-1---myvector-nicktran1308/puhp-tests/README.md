...
Only the *PuhPTests.h* header file need be included in your test code. Other header files and classes are automatically included when needed. Interesting classes are:
...
|Test          |Represents one unit test and contains convenience assertion methods that can adjust points earned. Can run multiple assertions per Test. **Do not** attempt to instantiate this class yourself; Instead, ask `Tests::createTest` to do it for you.|
|Leaderboard   |Allows you to set Leaderboard information, to be sent to Gradescope where students can compete for the best score. Send a Leaderboard object to a Test object with the `setLeaderboard` method.|
...
Here is a copy of a testing program made recently (2022-11-07). It grades units, outputs a *results.json* file for gradescope, outputs somewhat friendlier text for the student/user, uses the Leaderboard feature, and exits. It also features normalization of the final score to a target value, as well as normalization of individual test scores (which functions like weighting).
